<x-layout>
    ...
    <x-slot:header>
        <h1 class="text-center text-2xl font-bold">Edit Ticket</h1>
    </x-slot:header>
    
    <div>
        <div class="grid place-items-center">
            <form action="{{ route('tickets.update', ['project' => $project, 'ticket' => $ticket]) }}" method="POST" class="w-2/4">
                @csrf
                @method('PATCH')
                    ...
                    <div class="border-b border-gray-900/10 pb-12">
                        <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-6">
                            <x-form-field>
                                ...
                                <div class="mt-2">
                                    <x-form-input id="name" type="text" name="name" :value="old('name') ?? $ticket->name" placeholder="Enter ticket name" autocomplete="off" />
                            ...
                            <x-form-field>
                                ...
                                <div class="mt-2">
                                    <x-form-text name="description" id="description" :value="old('description') ?? $ticket->description" 
                                ...
                                <div class="mt-2">
                                    <select name="status_id" id="status_id"
                                        ...
                                        @foreach ($statuses as $status)
                                            <option 
                                                value="{{ $status->id }}"
                                                @selected($ticket->status_id == $status->id)
                                            >{{ $status->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mt-2">
                                    <select name="assignee_id" id="assignee_id" class="w-full appearance-none rounded-md bg-white py-1.5 pr-8 pl-3 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6">
                                        ...
                                        @foreach ($users as $user)
                                            <option 
                                                value="{{ $user->id }}"
                                                @selected($ticket->assignee_id == $user->id)
                                            >{{ $user->name }}</option>
                                        @endforeach
                                    </select>
